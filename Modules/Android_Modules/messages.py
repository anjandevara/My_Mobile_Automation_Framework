...
class TestMessagesScreenA(FindElements):
    ...
    def click_saved_bookmarks(self):
        self.more().click()
        self.find_element_by_resource_id(messages_screen.get("popup_bookmark")).click()

    def click_save_bookmarks_button(self):
        self.find_element_by_resource_id(messages_screen.get("toolbar_bookmark")).click()
    ...
    def fetch_bookmark_icons_count(self, expectedbookmarkicons):
        bookmark_conversation = self.find_elements_by_resource_id(messages_screen.get("bookmark_img"))
        bookmark_conversation_count = len(bookmark_conversation)
        assert bookmark_conversation_count == expectedbookmarkicons
    ...
    def fetch_all_message_of_a_contact(self):
        message_text_views = self.find_elements_by_resource_id(messages_screen.get("message_text_view"))
        return message_text_views

    def clear_notifications(self):
        self.driver.open_notifications()

    def resend_icon(self):
        resend_img = self.find_element_by_resource_id(messages_screen.get("resend_img"))
        return resend_img

    def resend_text(self):
        resend_text = self.find_element_by_resource_id(messages_screen.get("resend_text"))
        return resend_text

    def error_failed_msg(self):
        error_failedMsg = self.find_element_by_resource_id(messages_screen.get("error_failedMsg"))
        return error_failedMsg

    def msg_failed(self):
        msg_failed = self.find_element_by_resource_id(messages_screen.get("msg_failed"))
        return msg_failed
